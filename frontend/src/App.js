...
import { Home, History, TrendingUp, FileText, Dumbbell, Settings, Download } from 'lucide-react';
...
const AppShell = () => {
  const { colorMode, colorTheme } = useSettings();
  ...
  // Apply theme globally (Tailwind dark mode + CSS vars)
  useEffect(() => {
    ...
    root.setAttribute('data-theme', colorTheme || 'blue');
    ...
    >
      ...
      <Dialog open={showNavigationWarning} onOpenChange={setShowNavigationWarning}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-foreground">Unsaved Workout Data</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            ...
            <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-3">
              <p className="text-sm text-destructive font-semibold">⚠️ Your sets, weights, and reps will be lost!</p>
            </div>
          </div>
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-lg transition-all duration-200 whitespace-nowrap flex-shrink-0 ${
        active ? 'text-primary scale-105' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
      }`}
    >
