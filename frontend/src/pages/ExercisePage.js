...
/* ======================================================
   Rep scheme options (stored labels unchanged)
====================================================== */
const REP_SCHEMES = [
...
function ProgrammesPanel() {
  ...
  const availableExercises = useMemo(() => {
    const chosen = new Set(
      (editingProgramme?.exercises || []).map((e) => e?.id)
    );
    return (exercises || []).filter((ex) => !chosen.has(ex?.id));
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <h3
                      className="text-xl font-bold text-foreground mb-1 break-words whitespace-normal"
              ...
              {isExpanded && (
                <div className="px-4 pb-4 space-y-2 animate-fadeIn">
                  {(programme.exercises || []).map((exercise, index) => (
                    <div
                      key={`${exercise?.id || "ex"}_${index}`}
                      className="p-3 bg-muted/30 rounded-lg border border-border"
                    >
                      <div className="font-semibold text-foreground mb-1 break-words whitespace-normal">
                        {exercise.name}
                      </div>
                      <div className="text-sm text-muted-foreground space-y-1">
                        ...
                        <div>
                          Rest: {exercise.restTime}s • Scheme:{" "}
                          {String(exercise.repScheme || "")}
                        </div>
                        {exercise.notes ? (
                          <div className="text-xs mt-2 p-2 bg-card rounded border border-border break-words whitespace-pre-wrap">
                            {exercise.notes}
                          </div>
                        ) : null}
                      </div>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {availableExercises.map((exercise) => (
                      ...
                      >
                        <div className="min-w-0">
                          <div
                            className="font-semibold text-foreground break-words whitespace-normal leading-snug"
                            style={{ overflowWrap: "anywhere" }}
                          >
                            {exercise.name}
                          </div>
...
function ExercisesPanel() {
  ...
  const handleEditExercise = (exercise) => {
    ...
    const repScheme = normaliseRepScheme(exercise.repScheme);
  ...
  const filteredExercises = useMemo(() => {
    ...
    if (filterProgramme !== "all") {
      const prog = (programmes || []).find(
        (p) => toSafeToken(p?.type) === filterProgramme
      );
      const ids = new Set(
...
/* ======================================================
   COMBINED PAGE (with AppHeader)
====================================================== */
export default function ExercisePage() {
  const [tab, setTab] = useState("programmes"); // "programmes" | "exercises"

  return (
    <AppHeader
      title="Library"
      subtitle={tab === "programmes" ? "Programmes" : "Exercises"}
      rightIconSrc={`${process.env.PUBLIC_URL}/icons/icon-overlay-white-32-v1.png`}
      actions={
        <div className="w-full flex justify-center">
          <div className="w-full max-w-md grid grid-cols-2 rounded-xl overflow-hidden border border-white/25">
            <button
              type="button"
              onClick={() => setTab("programmes")}
              className={cx(
                "py-2 text-sm font-semibold text-center transition",
                tab === "programmes"
    ...
    >
      {tab === "programmes" ? <ProgrammesPanel /> : <ExercisesPanel />}
    </AppHeader>
  );
}