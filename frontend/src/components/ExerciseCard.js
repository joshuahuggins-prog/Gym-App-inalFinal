...
import { ChevronDown, ChevronUp, Timer, Video, Shuffle, Plus, Minus } from "lucide-react";
import { Button } from "./ui/button";
import { Input } from "./ui/input";
import { Textarea } from "./ui/textarea";
import { Badge } from "./ui/badge";
import { toast } from "sonner";
import { getVideoLinks, getWorkouts, getProgressionSettings } from "../utils/storage";
...
const normalizeGoalReps = (goalReps, count) => {
  ...
  return Array.from({ length: count }, (_, i) => base[i] ?? base[0] ?? 8);
};
...
const fmt1 = (n) => {
  ...
  return Number.isInteger(x) ? String(x) : round1(x).toFixed(1);
};

// ---------------------------
// Historical best set (weight + reps)
// ---------------------------
...
// ---------------------------
// History bests (weighted + assisted) for placeholder logic
// ---------------------------
const getHistoryBests = (exerciseKey) => {
  ...
  return {
    ...
    maxCompleteWeighted: maxCompleteWeighted === -Infinity ? null : maxCompleteWeighted,
    maxIncompleteWeighted: maxIncompleteWeighted === -Infinity ? null : maxIncompleteWeighted,
    bestCompleteAssist: bestCompleteAssist === Infinity ? null : bestCompleteAssist,
    bestIncompleteAssist: bestIncompleteAssist === Infinity ? null : bestIncompleteAssist,
  };
};
...
const ExerciseCard = ({
  ...
  const desiredSetsCount = useMemo(() => {
    ...
    return clampInt(liveLen > 0 ? liveLen : fallback, 1, 40);
  }, [exercise?.setsData, exercise?.sets]);

  const goalReps = useMemo(() => normalizeGoalReps(exercise?.goalReps, desiredSetsCount), [exercise?.goalReps, desiredSetsCount]);
  ...
  }, [exercise?.setsData, desiredSetsCount]);
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border bg-card border-border">
      {/* Header */}
      <div role="button" tabIndex={0} className="w-full text-left p-4 cursor-pointer select-none" onClick={handleHeaderToggle}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              {exercise?.repScheme && <Badge variant="secondary" className="text-[10px]">{exercise.repScheme}</Badge>}
              {pr?.weight != null && pr?.reps != null && <span className="text-foreground font-semibold">PR: {fmt1(pr.weight)} × {pr.reps}</span>}
            </div>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            <Button type="button" variant="ghost" size="sm" disabled={!hasVideo} onClick={(e) => { e.stopPropagation(); if (!hasVideo) return; onOpenVideo?.(exercise, videoLink); }}>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3">
          <div className="space-y-2">
            {sets.map((s, i) => (
              <div key={`${i}-${s.completed ? "c" : "n"}`} className="grid grid-cols-[60px_1fr_1fr_44px] gap-2 items-center">
                <span className="text-xs text-muted-foreground">Set {i + 1}</span>

                <Input type="text" inputMode="none" value={s.weight ?? ""} placeholder={suggestedWeightForSet(i)}
                  onFocus={() => openCustomNumberPad?.(exercise, i, "weight")}
                  onClick={() => openCustomNumberPad?.(exercise, i, "weight")} />

                <Input type="text" inputMode="none" value={s.reps ?? ""} placeholder={`${goalReps[i] ?? 8}`}
                  onFocus={() => openCustomNumberPad?.(exercise, i, "reps")}
                  onClick={() => openCustomNumberPad?.(exercise, i, "reps")} />

                <Button type="button" size="sm" variant={s.completed ? "default" : "outline"} onClick={(e) => { e.stopPropagation(); const next = [...sets]; next[i] = { ...s, completed: !s.completed }; pushUp(next); onSetComplete?.(exercise, next[i], false); }}>
                  ✓
                </Button>
              </div>
            ))}
          ...
          <Textarea value={notes} placeholder="Workout notes…" className="min-h-[70px]"
            onChange={(e) => setNotes(e.target.value)} onBlur={() => onNotesChange?.(exercise, notes)} />

          {/* Buttons */}
          <div className="flex flex-wrap gap-2">
            {onRestTimer && <Button type="button" variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); onRestTimer?.(exercise?.restTime ?? 120); }}><Timer className="w-4 h-4 mr-1" /> Rest</Button>}
            <Button type="button" variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); onAddSet?.(exercise); }}><Plus className="w-4 h-4 mr-1" /> Set</Button>
            <Button type="button" variant="outline" size="sm" disabled={sets.length <= 1} onClick={(e) => { e.stopPropagation(); const next = sets.slice(0, -1); pushUp(next); toast.success("Set removed", { duration: 1200 }); }}><Minus className="w-4 h-4 mr-1" /> Set</Button>
            <Button type="button" variant="outline" size="sm" onClick={(e) => {
              e.stopPropagation();
              const alts = EXERCISE_ALTERNATIVES?.[exercise?.id] || [];
              if (!alts.length) return toast.message("Alternatives", { description: "No alternatives saved for this exercise." });
              toast.message("Alternatives", { description: (<div className="mt-1 space-y-1">{alts.slice(0, 8).map((a, idx) => <div key={idx} className="text-sm">• {a}</div>)}</div>) });
            }}><Shuffle className="w-4 h-4 mr-1" /> Alternatives</Button>
          </div>
          ...
          {lastWorkoutData && (
            <div className="text-xs text-muted-foreground border border-border rounded-lg p-3 bg-muted/20">
              ...
              {(lastWorkoutData.sets || []).map((s2, idx) => <div key={idx}>Set {idx + 1}: {s2.weight} × {s2.reps}</div>)}
            </div>
          )}
        </div>
      )}
