...
const ExerciseCard = ({
  ...
  // Store sets as numbers internally, but render with text inputs (fixes Android "ghost 0")
  ...
  // Keep local "sets" array in sync if exercise.sets / goalReps change (important when you edit sets elsewhere)
  useEffect(() => {
    setSets((prev) => {
      const nextLen = Number(exercise.sets) || 1;

      const next = Array(nextLen)
        .fill(null)
        .map((_, i) => {
  ...
  const handleSetComplete = (setIndex) => {
    ...
    setSets(newSets);
    // Keep parent workoutData updated
  ...
  const handleWeightChange = (setIndex, raw) => {
    ...
    // Allow empty while typing: store 0 internally but render as empty string when 0
    const newWeight = toNumberOrZero(raw);
    newSets[setIndex].weight = newWeight;

    // Auto-calculate RPT weights for subsequent sets (only when first set has meaningful weight)
    if (exercise.repScheme === "RPT" && setIndex === 0) {
      const progressionSettings = getProgressionSettings();
      for (let i = 1; i < newSets.length; i++) {
        newSets[i].weight =
          newWeight > 0 ? calculateRPTWeights(newWeight, i + 1, progressionSettings) : 0;
      }
    }
  ...
  const handleRepsChange = (setIndex, raw) => {
    ...
    // Keep parent workoutData updated (so draft save + payload stays accurate)
    onWeightChange?.(exercise, newSets);
  };
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4 animate-fadeIn">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowVideoEdit(!showVideoEdit)}>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {/* Weight Input (text + inputMode fixes Android ghost '0') */}
                    <div className="flex-1">
                    ...
                    {/* Reps Input (text + inputMode fixes Android ghost '0') */}
                    <div className="flex-1">
      ...
      {showPlates && (
        <PlateCalculator weight={selectedWeight} open={showPlates} onClose={() => setShowPlates(false)} />
      )}
    </div>
  );
};
